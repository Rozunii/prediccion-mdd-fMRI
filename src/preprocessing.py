...
def preprocess_experiment(X, labels, splits, experiment_name,
    ...
    if apply_anova:
        ...
        print("  Ajustando SelectKBest (ANOVA) sobre train...")
        # ANOVA necesita ver todos los datos de train a la vez
        # Cargamos en batches y los apilamos — solo se hace una vez
        X_train_scaled_full = np.vstack([
            scaler.transform(X[batch_idx])
            for batch_idx in get_batches(idx_train)
        ])
        selector.fit(X_train_scaled_full, labels[idx_train])
        del X_train_scaled_full
    else:
        selector = None
    ...
    def transform_dataset(indices, dataset_name):
        ...
        for batch_idx in get_batches(indices):
            ...
            if apply_pca:
                ...
                batch_out = selector.transform(batch_scaled)
            else:
...
if __name__ == '__main__':
    ...
    archivos_cache = [
        os.path.join(save_dir, f'X_train_fc{sufijo}.npy'),
        os.path.join(save_dir, f'X_train_alff{sufijo}.npy'),
        os.path.join(save_dir, f'X_train_combined{sufijo}.npy'),
        os.path.join(save_dir, f'X_train_fc_anova{sufijo}.npy'),
        os.path.join(save_dir, f'X_train_combined_anova{sufijo}.npy'),
    ]
    cache_existe = all(os.path.exists(f) for f in archivos_cache)
    ...
    if cache_existe:
        ...
        print("[INFO] Resumen de datos cargados desde cache:")
        for name, exp in experiments.items():
            print(f"  {name.upper()}{sufijo}:")
            print(f"    Train: {exp['X_train'].shape}")
            print(f"    Val:   {exp['X_val'].shape}")
        ...
        try:
            ...
            print(f"\n[INFO] Indices del split guardados en: {save_dir}")

            # Guardar matrices procesadas
            print("\n[INFO] Guardando matrices procesadas...")
